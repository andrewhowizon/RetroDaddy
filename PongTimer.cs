...
public class PongTimer : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (Time.timeSinceLevelLoad < 4f) {
			...
			if (timer < 0.049f) {
				...
				if (manager) {
					manager.GetComponent<GameM> ().pongScore = GameObject.Find ("pongScore").GetComponent<PongScore> ().score;
					manager.GetComponent<GameM> ().pongCompleted = true;
					manager.GetComponent<GameM> ().overworldPos = new Vector3 (-40.84f, 2.852f, 12.17f);
